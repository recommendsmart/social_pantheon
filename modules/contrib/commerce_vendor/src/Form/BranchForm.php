...
/**
 * Form controller for Branch edit forms.
 *
...
class BranchForm extends ContentEntityForm {
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    switch ($status) {
      case SAVED_NEW:
        $this->messenger()->addMessage($this->t('Created the %label Branch.', [
          '%label' => $entity->label(),
        ]));
        break;

      default:
        $this->messenger()->addMessage($this->t('Saved the %label Branch.', [
          '%label' => $entity->label(),
        ]));
    }
    $form_state->setRedirect('entity.branch.canonical', ['branch' => $entity->id()]);
  }
