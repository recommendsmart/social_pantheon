 ...
 */
abstract class InlineEntityRevisionFormBase extends WidgetBase implements ContainerFactoryPluginInterface {
   ...
   *
   * @var \Drupal\inline_entity_revision_form\InlineRevisionFormInterface
   */
  ...
  protected function createInlineFormHandler() {
    if (!isset($this->inlineFormHandler)) {
      ...
      $this->inlineFormHandler = $this->entityTypeManager->getHandler($target_type, inline_entity_revision_form);
    }
  }
  ...
  protected function prepareFormState(FormStateInterface $form_state, FieldItemListInterface $items, $translating = FALSE) {
    $widget_state = $form_state->get(['inline_entity_revision_form', $this->iefId]);
    if (empty($widget_state)) {
      ...
      $form_state->set(['inline_entity_revision_form', $this->iefId], $widget_state);
    }
  }
   ...
   *
   * @see \Drupal\inline_entity_revision_form\TranslationHelper::initFormLangcodes()
   */
