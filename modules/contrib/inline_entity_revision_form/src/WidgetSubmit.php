...
namespace Drupal\inline_entity_revision_form;

use Drupal\Core\Form\FormStateInterface;
use Drupal\inline_entity_revision_form\Element\InlineEntityRevisionForm;
...
class WidgetSubmit {
  ...
  public static function doSubmit(array $form, FormStateInterface $form_state) {
    foreach ($form_state->get('inline_entity_revision_form') as &$widget_state) {
      $widget_state += ['entities' => [], 'delete' => []];
//       dsm( $widget_state['entities'] );
      foreach ($widget_state['entities'] as $delta => $entity_item) {
        if (!empty($entity_item['entity']) && !empty($entity_item['needs_save'])) {
          ...
          $handler = InlineEntityRevisionForm::getInlineFormHandler($entity->getEntityTypeId());
