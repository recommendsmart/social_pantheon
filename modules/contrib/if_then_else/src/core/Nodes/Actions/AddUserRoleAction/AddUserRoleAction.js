var VueAddUserRoleActionControl = {
  ...
  created() {
    if(drupalSettings.if_then_else.nodes.add_user_role_action.roles){
      //Fetching values of fields when editing rule page loads
      for (let option in drupalSettings.if_then_else.nodes.add_user_role_action.roles) {
        this.options.push({
          name: option,
          label: drupalSettings.if_then_else.nodes.add_user_role_action.roles[option]
        });
      }
    }
  }
...
class AddUserRoleActionControl extends Rete.Control {
  constructor(emitter, key) {
    super(key);
    this.component = VueAddUserRoleActionControl;
