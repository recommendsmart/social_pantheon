var VueSetFlagActionControl = {
  ...
  data() {
    return {
      type: drupalSettings.if_then_else.nodes.set_flag_action.type,
      class: drupalSettings.if_then_else.nodes.set_flag_action.class,
      name: drupalSettings.if_then_else.nodes.set_flag_action.name,
      classArg: drupalSettings.if_then_else.nodes.set_flag_action.classArg,
      value: [],
      options: [],
      selected_options: [],
      flag_bundles: [],
  ...
  methods: {
    entityValueChanged(value){
      if(value !== undefined && value !== null && value !== ''){
        ...
        var field_roles = drupalSettings.if_then_else.nodes.set_flag_action.flag_bundles
  ...
  mounted() {
    ...
    this.putData('type',drupalSettings.if_then_else.nodes.set_flag_action.type);
    this.putData('class',drupalSettings.if_then_else.nodes.set_flag_action.class);
    this.putData('name', drupalSettings.if_then_else.nodes.set_flag_action.name);
    this.putData('classArg', drupalSettings.if_then_else.nodes.set_flag_action.classArg);
  ...
  created() {
    if(drupalSettings.if_then_else.nodes.set_flag_action.flag_types){
      //setting list of all fields for a form when rule edit page loads.
      this.options = drupalSettings.if_then_else.nodes.set_flag_action.flag_types;
    }
  } 
};
class SetFlagActionControl extends Rete.Control {
  constructor(emitter, key, onChange) {
    super(key);
    this.component = VueSetFlagActionControl;
    this.props = { emitter, ikey: key, onChange};
  }
}
class SetFlagActionComponent extends Rete.Component {
  constructor() {
    var nodeName = 'set_flag_action';
  ...
  builder(eventNode) {
    ...
    eventNode.addControl(new SetFlagActionControl(this.editor, nodeName, handleInput()));
