...
namespace Drupal\if_then_else_group\core\Nodes\Actions\AddNodeToGroupAction;
...
/**
 * Add node to group class.
 */
class AddNodeToGroupAction extends Action {
  ...
  public static function getName() {
    return 'add_node_to_group_action';
  }
  ...
  public function registerNode(NodeSubscriptionEvent $event) {
    $event->nodes[static::getName()] = [
      'label' => $this->t('Add Node To Group'),
      'type' => 'action',
  ...
  public function process() {
    ...
    if (is_numeric($node)) {
      ...
      if (empty($node)) {
        $this->outputs['message'] = $this->t('Node id @node could not be added to group @group.', ['@node' => $this->inputs['node'], '@group' => $group_id])->render();
        $this->setSuccess(FALSE);
        return;
      }
    }
    elseif (!$node instanceof NodeInterface) {
      $this->outputs['message'] = $this->t('Node id @node could not be added to group @group.', ['@node' => $node->id(), '@group' => $group_id])->render();
      $this->setSuccess(FALSE);
      return;
    }
    ...
    if (!count($current_node)) {
      ...
      $this->outputs['message'] = $this->t('Node @node has been added to group @group.', ['@node' => $node->label(), '@group' => $group->label()])->render();
    }
    else {
      $this->outputs['message'] = $this->t('Node @node is already on group @group.', ['@node' => $node->label(), '@group' => $group->label()])->render();
      $this->setSuccess(FALSE);
      return;
    }
