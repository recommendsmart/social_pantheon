...
namespace Drupal\if_then_else_group\core\Nodes\Actions\RemoveUserFromGroupAction;
...
class RemoveUserFromGroupAction extends Action {
  ...
  public static function getName() {
    return 'remove_user_from_group_action';
  }
  ...
  public function registerNode(NodeSubscriptionEvent $event) {
    $event->nodes[static::getName()] = [
      'label' => $this->t('Remove User From Group'),
      'type' => 'action',
  ...
  public function process() {
    ...
    if (is_numeric($user)) {
      ...
      if (empty($user)) {
        $this->outputs['message'] = $this->t('User id @node could not be removed from group @group.', ['@node' => $this->inputs['user'], '@group' => $group_id])->render();
        $this->setSuccess(FALSE);
        return;
      }
    }
    elseif (!$user instanceof UserInterface) {
      $this->outputs['message'] = $this->t('User id @node could not be removed from group @group.', ['@node' => $user->id(), '@group' => $group_id])->render();
      $this->setSuccess(FALSE);
      return;
    }
    ...
    if ($member instanceof GroupMembership) {
      ...
      $this->outputs['message'] = $this->t('User @node has been remove from group @group.', ['@node' => $user->getUsername(), '@group' => $group->label()])->render();
    }
    else {
      $this->outputs['message'] = $this->t('User @node is not on group @group.', ['@node' => $user->getUsername(), '@group' => $group->label()])->render();
      $this->setSuccess(FALSE);
      return;
    }
  }
