...
class IndividualCRUDTest extends KernelTestBase {
  ...
  public function testIndividualType() {
    ...
    // Create.
    $individual_type = IndividualType::create(array('type' => $type));
    $this->assertTrue(isset($individual_type->type) && $individual_type->type == $type, 'New individual type exists.');
    ...
    $this->assertEquals(SAVED_NEW, $individual_type->save(), 'Individual type saved.');

    // Load.
    $individual_type_load = IndividualType::load($type);
    $this->assertEquals($individual_type->type, $individual_type_load->type, 'Loaded individual type has same type.');
    $this->assertEquals($individual_type->name, $individual_type_load->name, 'Loaded individual type has same name.');
    $this->assertEquals($individual_type->description, $individual_type_load->description, 'Loaded individual type has same description.');
    ...
    $this->assertTrue($individual_type->name == $individual_type_labels[$individual_type->type]);
  ...
  public function testIndividual() {
    ...
    $this->assertEquals(SAVED_NEW, $individual_one->save(), 'Individual saved.');

    // Assert default labels.
    $this->assertEquals('Nameless #' . $individual_one->id(), $individual_one->label());
  ...
  public function testActionPlugins() {
    ...
    $seller_individual = Individual::create(['type' => 'seller', 'name' => ['title'=>'Will', 'family' => 'Smith']]);
    ...
    $relation_type = RelationType::create([
      'id' => 'crm_member',
      'source_bundles' => ['crm_core_individual:*', 'crm_core_organization:*'],
    ...
    $meeting_activity = Activity::create([
      ...
      'activity_participants' => [$individual_customer_2, $individual_customer_3],
    ]);
