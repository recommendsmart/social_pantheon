...
class ConditionTypeManager extends DefaultPluginManager {
  ...
  /**
   * Map of PluginFormInterface lookup results.
   *
   * @var array
   */
  protected $pluginFormExists;
  ...
  /**
   * Check if 'condition_type' plugin implements PluginFormInterface.
   */
  public function isPluginForm($condition_type) {
    if (!isset($this->pluginFormExists[$condition_type])) {
      $this->pluginFormExists[$condition_type] = $this->createInstance($condition_type) instanceof PluginFormInterface;
    }
    return $this->pluginFormExists[$condition_type];
  }
