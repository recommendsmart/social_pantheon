  ...
  SmartContentManager.plugin.client.decisionAgentClientSide = {
    process: function (Decision) {
      ...
      for (var i = 0; i < Decision.variations.length; i++) {
        ...
        // If after processing conditions, variation still not appeased,
        // restrict later variants from processing Decision.
        if (!Variation.appeased) {
    ...
    execute: function (Decision) {
      if (Decision.params.length === 3) {
        ...
        if (Decision.context.length !== 0) {
          var query = Object.keys(Decision.context).reduce(function (a, k) {
            a.push(k + '=' + encodeURIComponent(Decision.context[k]));
            return a
          }, []).join('&');
          var url = url + '?' + query;
        }
