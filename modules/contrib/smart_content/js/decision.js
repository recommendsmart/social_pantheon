(function ($) {
        ...
        for (var d in drupalSettings.smartContentDecisions) {
          if (drupalSettings.smartContentDecisions[d].name == id) {
            var a = [].filter.call(this.attributes, function (at) {
              return /^data-context-/.test(at.name);
            });
            var context = {};
  ...
  // }
  //
  // Drupal.smart_content.model.Decision.prototype.setVariation = function (id,
  // override) { if(typeof override == undefined) { override = false; } }
  ...
  //     var found = false;
  //     $('[data-smart-content-decision]',
  // context).once('ajax-build').each(function () { found = true; var $wrapper
  // = $(this);
  // Drupal.SmartDecisionManager.addDecision($wrapper.attr('data-smart-content-decision'));
  // }); if(found) { Drupal.SmartDecisionManager.execute(); } } }
  // Drupal.SmartDecisionManager = {} Drupal.smart_content =
  // Drupal.smart_content || {}; Drupal.smart_content.decisions =
  // Drupal.smart_content.decisions || [];
  // Drupal.SmartDecisionManager.addDecision = function (id) {
  // if(Drupal.smart_content.decisions.indexOf(id) < 0) {
  // Drupal.smart_content.decisions.push(id); } }  Drupal.SmartDecisionManager.execute  = function () { var decisions = Drupal.smart_content.decisions; for (d=0; d < decisions.length;d++) { var entity_id = decisions[d].split(/\.(.+)/)[1]; var query = {data: {'condition_default:browser.language' : 'en', test2 : 'testttyeah'}}; var url = '/ajax/smart-content/decision/config/' + entity_id; var ajaxObject = new Drupal.ajax({ url: url, progress: false, success: function (response, status) { for (var i in response) { if (response.hasOwnProperty(i) && response[i].command && this.commands[response[i].command]) { this.commands[response[i].command](this, response[i], status); } } } }); ajaxObject.options.data['context'] = $.param(query);  ajaxObject.execute(); } }


  // Drupal.SmartDecisionManager = function () {
  //   this.decisions = (typeof this.decisions === 'undefined') ? [] :
  // this.decisions; } Drupal.SmartDecisionManager.prototype = {  /** * Command
  // to insert new content into the DOM. * * @param {Drupal.Ajax} ajax *
  // {@link Drupal.Ajax} object created by {@link Drupal.ajax}. * @param
  // {object} response *   The response from the Ajax request. * @param
  // {string} response.data *   The data to use with the jQuery method. *
  // @param {string} [response.method] *   The jQuery DOM manipulation method
  // to be used. * @param {string} [response.selector] *   A optional jQuery
  // selector string. * @param {object} [response.settings] *   An optional
  // array of settings that will be used. * @param {number} [status] *   The
  // XMLHttpRequest status. */ addDecision: function (id) {  } }
