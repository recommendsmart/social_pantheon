...
class SmartBlock extends BlockBase {
  ...
  public function build() {
    ...
    /** @var \Drupal\smart_content\Entity\SmartVariationSet $entity */
    if ($entity = \Drupal::entityTypeManager()
      ->getStorage('smart_variation_set')
      ->load($configuration['variation_set'])) {
      $build = [
  ...
  public function buildWidget(array &$element, FormStateInterface $form_state, array &$complete_form) {
    ...
    // Load the existing configuration form if already exists(rebuild),
    // otherwise return empty array.
    $variation_set_form = !empty($element['variation_set_config']) ?
      $element['variation_set_config'] : [];
    // Create subform state.
    $variation_set_form_state = SubformState::createForSubform($variation_set_form, $element, $form_state);
    // Retrieve the entity edit form object, and set the entity and build the
    // form.
    $variation_set_form = \Drupal::entityTypeManager()
      ->getFormObject('smart_variation_set', 'edit')
      ->setEntity($entity)
      ->buildForm($variation_set_form, $variation_set_form_state);

    // Remove action buttons from SmartVariationSetForm, block submit will
    // trigger instead..
    unset($variation_set_form['actions']);
    // Embed form.
    $element['variation_set_config'] = $variation_set_form;
    // Set the entity ID value.
    $element['variation_set'] = [
      '#type' => 'value',
      '#value' => $entity->id(),
    ];
    return $element;
