  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      ...
      if ($business->access('view')) {
        $form_state->setRedirect('entity.drm_core_business.canonical', ['drm_core_business' => $business->id()]);
      }
      else {
        $form_state->setRedirect('entity.drm_core_business.collection');
      }
    }
    elseif ($status == SAVED_NEW) {
      drupal_set_message($this->t('The business %name has been added.', ['%name' => $business->label()]));
      \Drupal::logger('drm_core_business')->notice('Added business %name.', ['%name' => $business->label()]);
      $form_state->setRedirect('entity.drm_core_business.collection');
    }
  }
