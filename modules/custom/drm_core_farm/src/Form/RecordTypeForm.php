...
class RecordTypeForm extends EntityForm {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    /* @var \Drupal\drm_core_farm\Entity\RecordType $type */
    $type = $this->entity;
    ...
    $form['type'] = array(
      ...
      '#machine_name' => array(
        'exists' => 'Drupal\drm_core_farm\Entity\RecordType::load',
        'source' => array('name'),
      ),
...
//    $primary_fields = variable_get('drm_core_farm_default_primary_fields', $default_primary_fields);
    $primary_fields = $default_primary_fields;
    $options = array();
    if (isset($type->type)) {
      /* @var \Drupal\Core\Field\FieldDefinitionInterface[] $instances */
      $instances = \Drupal::service('entity_field.manager')->getFieldDefinitions('drm_core_record', $type->type);
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    $t_args = array('%name' => $type->label(), 'link' => \Drupal::url('entity.drm_core_record_type.collection'));
    ...
    elseif ($status == SAVED_NEW) {
      ...
      \Drupal::logger('drm_core_record')->notice('Added record type %name.', $t_args);
    }

    $form_state->setRedirect('entity.drm_core_record_type.collection');
  }
