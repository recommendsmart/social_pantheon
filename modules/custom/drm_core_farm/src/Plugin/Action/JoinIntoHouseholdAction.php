...
use Drupal\drm_core_farm\Entity\Contact;
...
/**
 * Merges 2 or more farms into household farm.
 *
 * @Action(
...
class JoinIntoHouseholdAction extends ConfigurableActionBase implements ContainerFactoryPluginInterface {
   ...
   */
  const RELATION_TYPE_HOUSEHOLD = 'drm_member';
  ...
  public function executeMultiple(array $objects) {
    if (!isset($this->configuration['household'])) {
      $this->configuration['household'] = Farm::create(['type' => 'household']);
    }
    // Saving household only now because user can click "Cancel" on confirmation
    // page(if he/she will notice that selected wrong farms).
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    $household = Farm::create(['type' => 'household', 'name' => 'Fam. Smith']);
