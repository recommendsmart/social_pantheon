...
namespace Drupal\drm_core_farm\Plugin\Action;

use Drupal\Core\Action\ConfigurableActionBase;
use Drupal\Core\Session\AccountInterface;
use Drupal\Core\Access\AccessResult;
use Drupal\Core\Form\FormStateInterface;
use Drupal\drm_core_activity\Entity\Activity;
use Drupal\drm_core_farm\Entity\Farm;
...
class MergeFarmsAction extends ConfigurableActionBase implements ContainerFactoryPluginInterface {
  ...
  public function executeMultiple(array $objects) {
    ...
    foreach (array_keys($wrappers) as $farm_id) {
      ...
      $this->pathAliasStorage->save('/drm-core/farm/' . $primary_farm->id(), '/drm-core/farm/' . $farm_id);
      if ($this->moduleHandler->moduleExists('drm_core_activity')) {
        // Replacing participant in existing activities.
        $query = $this->entityQuery->get('drm_core_activity');
        $activities = $query->condition('activity_participants.target_id', $farm_id)
          ->condition('activity_participants.target_type', 'drm_core_farm')
          ->execute();
      ...
      if ($this->moduleHandler->moduleExists('relation')) {
        ...
        $relations = $query->condition('endpoints.entity_type', 'drm_core_farm', '=')
              ->condition('endpoints.entity_id', $farm_id, '=')
              ->execute();
        foreach ($relations as $relation_info) {
          $endpoints = array(
            array('entity_type' => 'drm_core_farm', 'entity_id' => $primary_farm->id()),
          );
    ...
    $this->entityTypeManager->getStorage('drm_core_farm')->delete($wrappers);
  ...
  public function buildConfigurationForm(array $form, FormStateInterface $form_state) {
    ...
    if (count(array_unique($farm_types)) != 1) {
      ...
      $form_state->setRedirect('entity.drm_core_farm.collection');
    }
    else {
      $form['table'] = array(
        '#type' => 'table',
      ...
      $field_instances = $this->entityFieldManager->getFieldDefinitions('drm_core_farm', reset($farm_types));
    ...
    $form['#attached']['library'][] = 'drm_core_farm/drupal.drm_core_farm.merge-farms';
