 ...
 *
 * Provides a form for the RecordType entity.
 *
...
class RecordTypeForm extends BundleEntityFormBase {
  ...
  public function form(array $form, FormStateInterface $form_state) {
    ...
    /* @var \Drupal\crm_core_data\Entity\RecordType $type */
    $type = $this->entity;
    ...
    $form['id'] = [
      ...
      '#machine_name' => [
        'exists' => 'Drupal\crm_core_data\Entity\RecordType::load',
        'source' => ['label'],
      ],
    ...
    if (isset($type_id)) {
      ...
      $instances = \Drupal::service('entity_field.manager')->getFieldDefinitions('crm_core_record', $type_id);
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    if ($status == SAVED_UPDATED) {
      drupal_set_message($this->t('The record type %name has been updated.', ['%name' => $type->label()]));
    }
    elseif ($status == SAVED_NEW) {
      drupal_set_message($this->t('The record type %name has been added.', ['%name' => $type->label()]));
      \Drupal::logger('crm_core_record')->notice('Added record type %name.', ['%name' => $type->label()]);
    }
